...
func TestEditaUmAlunoHandler(t *testing.T) {
	...
	resposta := httptest.NewRecorder()
	r.ServeHTTP(resposta, req)
	var alunoMockAtualiazdo models.Aluno
	json.Unmarshal(resposta.Body.Bytes(), &alunoMockAtualiazdo)
	assert.Equal(t, "47123456789", alunoMockAtualiazdo.CPF)
	assert.Equal(t, "123456700", alunoMockAtualiazdo.RG)
	assert.Equal(t, "Nome do Aluno Teste", alunoMockAtualiazdo.Nome)
}
